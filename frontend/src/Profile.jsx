...
function Profile() {
    ...
    const loadUserHandles = async (email) => {
        try {
            const dbResponse = await axios.get(`http://127.0.0.1:5000/api/get-handles?email=${email}`);
        ...
        try {
            if (cfHandle) {
                try {
                    ...
                    const infoRes = await axios.get(`http://127.0.0.1:5000/api/user/cf/${cfHandle}`);
                    cfProfileData = infoRes.data;

                    // Fetch status for timeline and rating counts
                    const cfResponse = await axios.get(`http://127.0.0.1:5000/api/user/cf-status/${cfHandle}`);
            ...
            if (ltHandle) {
                try {
                    const ltResponse = await axios.get(`http://127.0.0.1:5000/api/user/lt/${ltHandle}`);
    ...
    const handleLinkPlatform = async (e, platform, handleValue) => {
        ...
        setLoading(true);
        try {
            await axios.post('http://127.0.0.1:5000/api/save-handle', { email: userSession.email, handle: handleValue, platform: platform });
    ...
    const handleDisconnectPlatform = async (platform) => {
        setLoading(true);
        try {
            await axios.post('http://127.0.0.1:5000/api/save-handle', { email: userSession.email, handle: '', platform: platform });
